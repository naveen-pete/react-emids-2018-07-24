  ...
  return (
    <div className={`card text-white mb-3 ${done ? 'bg-success' : 'bg-info'}`}>
      ...
      <div className="card-footer">
        <button className="btn btn-sm mr-2" onClick={() => props.onToggle(id)}>Toggle Status</button>
        { done && <button className="btn btn-warning btn-sm" onClick={() => props.onDelete(id)}>Delete</button> }
      </div>
    </div>  
  );
}
